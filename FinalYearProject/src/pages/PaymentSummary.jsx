...
const createTicket = async (e) => {
  ...
  // Set default values for name, email, and contact number if they are empty
  const defaultName = user ? user.name : '';
  const defaultEmail = user ? user.email : '';
  const defaultContactNo = '';

  // Update the ticket details with default values if the corresponding fields are empty
  const updatedTicketDetails = {
    ...
    ticketDetails: {
      ...
      contactNo: details.contactNo || defaultContactNo,
    }
  };
  ...
  try {
    ...
    updatedTicketDetails.ticketDetails.qr = qrCode;

    // Post the ticket details to the backend
  ...
  return <Navigate to={'/wallet'} />
    ...
    return (
      ...
      <div className="float-right bg-blue-100 w-1/4 p-5 mt-8 mr-12">
          <h2 className="text-xl font-bold mb-8">Ticket Summary</h2>
